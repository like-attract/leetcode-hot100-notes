...
class Solution:
    def threeSum(self, nums: list[int]) -> list[list[int]]:
        cnt = Counter(nums) # 统计数组各个数的出现次数
        keys = sorted(cnt.keys()) # keys：数 values: 次数
        res = []

        for i,x in enumerate(keys): #keys：从小到大排列 无重复数字
            for j in range(i,len(keys)): #i:0-len, j:i-len
                y = keys[j] #这里天然包括了两个x
                z = -x-y # 根据x+y+z=0
                if z<y:continue
